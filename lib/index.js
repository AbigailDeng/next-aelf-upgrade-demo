...
const getPkgObj = (cloneResponse) => {
  const pkgCurr = (0, _fs.readFileSync)(
...
const init = async () => {
  ...
  const cloneResponse = await _Helper.GitHelper.CloneAndGetPath(
    sourcePath,
    branch
  );
  ...
  const upgradeDep = _Helper.DiffHelper.diffObjWithKeys(
    curr,
    prev,
    _Constants.UPGRADE_DEP
  );
  const upgradeDevDep = _Helper.DiffHelper.diffObjWithKeys(
    curr,
    prev,
    _Constants.UPGRADE_DEV_DEP
  );
  const depResponse = _Helper.BinaryHelper.upgradeChangedPkg(upgradeDep);
  const devDepResponse = _Helper.BinaryHelper.upgradeChangedPkg(
    upgradeDevDep,
    true
  );
