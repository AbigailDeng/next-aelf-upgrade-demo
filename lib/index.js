...
const getPkgObj = cloneResponse => {
  const pkgCurr = (0, _fs.readFileSync)(`${cloneResponse.trim()}/web/next-aelf/package.json`, "utf-8");
...
const init = async () => {
  ...
  const cloneResponse = await _Helper.GitHelper.CloneAndGetPath(sourcePath, branch);
